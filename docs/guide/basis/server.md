...
在 `admin-element-vue-typescript` 中，一个完整的前端 UI 交互到服务端处理流程是这样的：
...
`@/utils/request.ts` 是基于 [Axios](https://github.com/axios/axios) 的封装，便于统一处理 POST，GET 等请求参数，请求头，以及错误提示信息等。具体可以参看 [@/utils/request.ts](https://github.com/lqsong/admin-element-vue/blob/typescript.v2/src/utils/request.ts)。
它封装了全局 `request拦截器`、`response拦截器`、`统一的错误处理`、`统一做了超时处理`、`baseURL设置等`。

## 一个表单提交例子：

[代码](https://github.com/lqsong/admin-element-vue/tree/typescript.v2/src/views/pagesample/form/basic)
